...
//			if("article".equals((request.getSession().getAttribute("fckSavePath")))){//文章的话添加到一个文件夹中 ，在feceditor.properties文件中
//				typeDirPath = otherFilePath + "/static/images/";
//			}else if("exam".equals((request.getSession().getAttribute("exam")))){//文章的话添加到一个文件夹中 ，在feceditor.properties文件中
//				typeDirPath = otherFilePath + "/static/images/";
//			}
//			else{
...
//			}
			...
			if (!currentDir.exists())
				...
				try {
					...
					// We upload only one file at the same time,上传图片
					...
					filename = UUID.randomUUID() + "." + extension;//解决文件中文问题，唯一标识
					...
					if (!ExtensionsHandler.isAllowed(resourceType, extension))
						...
						int counter = 1;
						while (pathToSave.exists()) {//判断是否已存在，存在则重命名
						...
						if (Utils.isEmpty(newFilename)){//文件名未重复的情况下
							if(fckSavePath != null && !fckSavePath.trim().equals("")){
								String temp = "http://import.highso.org.cn/upload" + "/"+fckSavePath+"/" + filename;
