...
function dTree(objName, iPath, formId) {
	...
	this.icon = {
		...
		root : iPath + '/back/images/dtree/base.gif',

		folder : iPath + '/back/images/dtree/folder.gif',

		folderOpen : iPath + '/back/images/dtree/folderopen.gif',

		node : iPath + '/back/images/dtree/page.gif',

		empty : iPath + '/back/images/dtree/empty.gif',

		line : iPath + '/back/images/dtree/line.gif',

		join : iPath + '/back/images/dtree/join.gif',

		joinBottom : iPath + '/back/images/dtree/joinbottom.gif',

		plus : iPath + '/back/images/dtree/plus.gif',

		plusBottom : iPath + '/back/images/dtree/plusbottom.gif',

		minus : iPath + '/back/images/dtree/minus.gif',

		minusBottom : iPath + '/back/images/dtree/minusbottom.gif',

		nlPlus : iPath + '/back/images/dtree/nolines_plus.gif',

		nlMinus : iPath + '/back/img/nolines_minus.gif'
...
dTree.prototype.node = function(node, nodeId) {
	// alert("nodeId = " + nodeId + " | parentId = " + node.pid + " | nodeId = "
	// + node.id);
	...
	if (this.config.useCheckBox && nodeId != 0) {
		if (node.pid == 0) {
			...
			this.parEnum = this.parEnum.substring(0, pL
							+ (node.pid + "-").length)
					+ node.id + "-";
			// alert("parEnum2 = " + this.parEnum);
		}

		str += '<input type="checkbox" name="id" id="c' + this.parEnum
				+ '" onClick="caBox(\'' + this.formId + '\',\'c' + this.parEnum
				+ '\')" value="' + node.id + '" class="cx"/>';
	} else {
		this.parEnum += node.id + "-";
	}
...
function caBox(formId, regx) {
	if (document.getElementById(regx).checked) {
		...
		for (j = 0; j < regxArray.length; j++) {
			// alert("sssswww");
			var pDiv = regx
					.substring(0, regx.indexOf(regxArray[regxArray.length - j
									- 1]
									+ "-"));
		...
		for (j = 0; j < regxArray.length - 1; j++) {
			if (!isCheckedByRec(form, regx.substring(0,
							regx.indexOf(regxArray[regxArray.length - j - 2]
									+ "-")))) {
				var pDiv = regx.substring(0, regx
								.indexOf(regxArray[regxArray.length - j - 2]
										+ "-"));
				if (document.getElementById(pDiv))
...
function isCheckedByRec(form, regx) {
	...
	for (var i = 0; i < form.elements.length; i++) {
		...
		if (element.name == "id" && element.type == 'checkbox') {
			// isChecked
			if (element.id.indexOf(regx) != -1 && element.checked
					&& element.id != regx) {
				return true;
			}
		}
