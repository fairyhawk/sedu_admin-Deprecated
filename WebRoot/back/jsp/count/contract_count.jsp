	...
	<head>
		...
		$().ready(function(){
			...
			var sumAllOrder=0,sumDtoOrder=0,sumDtoOrderNo=0,sumBankOrder=0,sumBankOrderNo=0,sumQuickOrder=0,sumPerOne=0,sumPerTwo=0,
   			countOne=0,countTwo=0,sumTruefriendsOrder=0,sumRemittanceOrder=0,sumRemittanceOrderNo=0,sumTruefriendsOrderNo=0,
   			sumQuickOrderNo=0,sumInternetOrder=0,sumInternetOrderNo=0,sumOrderNo=0,sumGiftOrder=0,sumAfterAmount=0,sumRefund=0,sumUnionpayOrder=0,sumUnionpayOrderNo=0;
   			$("label").each(function(){
				...
				if($(this).attr("class")=="sumUnionpayOrder"){
					sumUnionpayOrder+=parseInt($(this).text().replace(/\s+$|^\s+/g,""));
				}
				if($(this).attr("class")=="sumUnionpayOrderNo"){
					sumUnionpayOrderNo+=parseInt($(this).text().replace(/\s+$|^\s+/g,""));
				}
			...
			$("#test").children().eq(10).text(sumUnionpayOrder);
			$("#test").children().eq(11).text(sumUnionpayOrderNo);
			$("#test").children().eq(12).text(sumTruefriendsOrder);
			$("#test").children().eq(13).text(sumTruefriendsOrderNo);
			$("#test").children().eq(14).text(sumRemittanceOrder);
			$("#test").children().eq(15).text(sumRemittanceOrderNo);
		...
		function setL(num){
			...
			$("input[name=endTime]").val(""); 
		 	$("#ser").attr("action","<%=contextPath%>/count/contractCount!getContractInfo.action?l="+num);
		 	$("#ser").submit();
		}
		function SpaceKc(h){
			for(var i=0;i<5;i++)
		...
		function search(){
			  ...
			  if(startTime>endTime){
			 	...
			 	document.search.action="<%=contextPath%>/count/contractCount!getContractInfo.action";
		...
		function resetKey() {
			$("input[name=startTime]").val("");
			$("input[name=endTime]").val(""); 
			$("input[name=queryContractCountCondition.webFrom]").val("");
			$("input[name=queryContractCountCondition.webAgnet]").val("");
				...
				<td>
					<div class="spa_tabcon dispblock spa_tabconbor">
						<form id="ser" name="search" method="post">
							<!-- 域名来源 -->
							<s:if test="queryContractCountCondition.contractFromUrl!=null">
								<input type="hidden" value=<s:property value="queryContractCountCondition.contractFromUrl"/> id="cfu"/>								
							</s:if>
							<s:else>
								<input type="hidden" value=0 id="cfu"/>	
							</s:else>
							<!-- 推广来源 -->
							<s:if test="queryContractCountCondition.webFrom!=null">
								<input type="hidden" value=<s:property value="queryContractCountCondition.webFrom"/> id="wf"/>								
							</s:if>
							<s:else>
								<input type="hidden" value=0 id="wf"/>	
							</s:else>
							<!-- 订单渠道 -->
							<s:if test="queryContractCountCondition.webAgnet!=null">
								<input type="hidden" value=<s:property value="queryContractCountCondition.webAgnet"/> id="wa"/>								
							</s:if>
							<s:else>
							...
							<table width="75%" border="0" cellspacing="1" cellpadding="0"
								...
								<tr align="left">
									<td>
										<s:hidden name="queryContractCountCondition.currentPage" value="1" />
										开始时间：
								...
								<tr>
									<td>
										域名来源：
										<select name="queryContractCountCondition.contractFromUrl"
											id="contractFromUrl"
											style="width: 155px">
											<option value="" id="op3">
												---请选择---
									...
									<td>
										...
										<s:select id="subjectId"
											name="queryContractCountCondition.subjectId"
											list="allSubject" listKey="subjectId" listValue="subjectName"
											headerValue="-查询全部-" headerKey="0" style="width: 155px">
										</s:select>
									</td>
								</tr>
								<tr>
									<td>
										推广来源：
										<input type="text" name="queryContractCountCondition.webFrom"
											id="webFrom" />
									</td>
									<td>
										订单渠道：
										<input type="text" name="queryContractCountCondition.webAgnet"
											id="webAgnet" />
									</td>
								</tr>
								<tr>
						...
						<table width="99%" border="0" cellspacing="1" cellpadding="0"
							...
							<s:iterator id="contractCountDto" value="contractcountNewDTOlist">
								<tr>
									<td height="28" align="center" bgcolor="#FFFFFF"
										style="font-weight: bold;">
										<s:property value="#contractCountDto.subName" />
									</td>
									<td height="28" align="center" bgcolor="#FFFFFF">
									...
									<td height="28" align="center" bgcolor="#FFFFFF">
									<label class="sumUnionpayOrder">
										<s:if test="#contractCountDto.unionpayOrder!=null">
										<s:property value="#contractCountDto.unionpayOrder" />
										</s:if>
										<s:else>0</s:else>
									</label>
									</td>
									<td height="28" align="center" bgcolor="#FFFFFF">
									<label class="sumUnionpayOrderNo">
										<s:if test="#contractCountDto.unionpayOrderNo!=null">
										<s:property value="#contractCountDto.unionpayOrderNo" />
										</s:if>
										<s:else>0</s:else>
										</label>
									</td>
