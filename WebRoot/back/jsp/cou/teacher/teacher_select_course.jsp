...
<html>
	<head>
		...
		function chooseCourse() {
			if(window.confirm("您确定要选择已勾选中的课程吗？")) {
				...
				$.ajax({
					url : "<%=contextPath %>/cou/teacher!chooseCourse.action",  
					data : {'ids' : ids, 'teacher.tcId' : $("#tcId").val()},  // 参数  
					type : "post",  
					cache : false,  
					error: function(){ 
						alert('error');      
		...
		function unchoose(btn,id) {
			if(window.confirm("您确定要取消已选择课程吗？")) {
				$.ajax({  
					url : "<%=contextPath %>/cou/teacher!unchoose.action",  
					data : {'ids' : [id], 'teacher.tcId' : $("#tcId").val()},  // 参数  
					type : "post",  
					cache : false,  
					error: function(){ 
						alert('error');      
		...
		function onchangeFirstSort(pId){
			if(pId != -1){
				document.getElementById('queryTeacherCondition.sortId').value = pId;
			}
			$.ajax({  
	...
	<body>	
		<div class="main_right">
			...
			<form name="prekpointform" id="prekpointform" action="<%=contextPath %>/cou/teacher!initSelectCourse.action?queryTeacherCondition.currentPage=1" method="post">
				<input type="hidden" name="queryTeacherCondition.tcId" id="tcId" value="<s:property value="queryTeacherCondition.tcId"/>"/>
				<table class="tables" border="0" cellpadding="0" cellspacing="1" style="width:600px;">
					...
					<tr align="center">
						<td colspan="2">
							<input type="hidden" name="queryTeacherCondition.sortId" id="querySortId"
								value="<s:property value="queryTeacherCondition.sortId"/>"></input>
							<s:submit value="提交"  />
						</td>
					</tr>
				</table>
			...
			<form action="<%=contextPath%>/cou/teacher!initSelectCourse.action" method="post" name="courseForm" id="courseForm">
				<input type="hidden" name="queryTeacherCondition.tcId" value="<s:property value="queryTeacherCondition.tcId"/>"/>
				<table class="tables">
					<tr>
						<td>
							<s:hidden name="queryTeacherCondition.currentPage" value="1" />
							<input type="hidden" name="queryTeacherCondition.sortId"
								value="<s:property value="queryTeacherCondition.sortId"/>"></input>
							<table>
								<tr>
									<td>
										按课程名检索
										<input type="text" name="queryTeacherCondition.courseName" id="queryTeacherCondition.courseName"
											value="<s:property value="queryTeacherCondition.courseName"/>" />
									</td>
									<td>
				...
				<table class="tables">
					<tr>
						<td colspan="10">
							<input type="button" value="选课" onclick="chooseCourse()"/>
					...
					<s:iterator value="page.pageResult" id="course">
						<tr>
							...
							<td>
								<input type="button" value="取消选课" <s:property value="status==1?'':'disabled'"/> onclick="unchoose(this, <s:property value="courseId"/>)"/>
							</td>
						</tr>
					</s:iterator>
					<tr>
