...
<link rel="stylesheet" type="text/css" media="screen"	href="<%=contextPath%>/back/style/css_body.css" />
		...
		<script type="text/javascript">
			...
			function onchangecallback(result){   
					//alert(data);		
				document.getElementById('sSortId').options.length = 0;
				var str="";  
	...
	<body>
<div>
	<form name="coursesortupdate" id="coursesortupdate" action="<%=contextPath %>/cou/coursesort!updateCoursesort.action" enctype="multipart/form-data" method="post" onsubmit="checkSubmit();">
	<table width="100%" border="0" cellspacing="0"  cellpadding="0" class="lists">
		<tr >
			...
			<td class="lists_top">
				<font class="lists_fleft">课程分类更新</font>
		...
		<tr>
			...
			<table width="100%" border="0" cellspacing="1" cellpadding="0" class="lists_info">
				...
				<tr>
					...
					<td class="lists_tleft">
						<input type="text" name="sort.coursesortName" id="sort.coursesortName" class="required" value="<s:property value="sort.coursesortName"/>"/>
					</td>
				</tr>
				...
				<tr>
					...
					<td class="lists_tleft">
						<input type="file" name="sortPic" id="sortPic"
							 onchange="changeSortPic(this)"/>
						<div class="uploadPic" id="mainpicDiv"></div>
						<img id="mainpicImg" src="<%=contextPath %>/back/upload/coursesort/<s:property value='sort.sortPicUrl'/>"/>
					</td>
				</tr>
				...
				<tr>
					...
					<td class="lists_tleft">
						...
						<s:if test="sort.status == 1">
							<input type=radio name="sort.status" value=0  />网站分类
	    					<input type=radio name="sort.status" value=1 checked/>冻结
	    					<input type=radio name="sort.status" value=3 />内部学员
						</s:if>
						<s:if test="sort.status == 3">
							<input type=radio name="sort.status" value=0  />网站分类
	    					<input type=radio name="sort.status" value=1 />冻结
	    					<input type=radio name="sort.status" value=3 checked/>内部学员
						</s:if>
					</td>
				</tr>
				...
				<tr>
					...
					<td class="lists_tleft">
						<input type="text" name="sort.sortNum" id="sort.sortNum" class="{required:true,min:0}" value="<s:property value="sort.sortNum"/>"/>
					...
					<td colspan="2">
					   <s:hidden name="sort.level" id="sort.level"></s:hidden>
						<s:hidden name="searchKey" id="searchKey" ></s:hidden>
					 	<s:hidden name="sort.coursesortId" id="sort.coursesortId" ></s:hidden>
