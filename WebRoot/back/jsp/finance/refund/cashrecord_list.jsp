...
function fKuanOne(){
 ...
 /* var vd=1;
  ...
  function cancel(){
	...
	function openSellWayList(cusId,cashId) { 
																																																											
		window.open ("<%=contextPath%>/finance/backCashRecord!showSellWayList.action?customer.cusId="+cusId+"& cashRecord.id="+cashId+"&querySellWayCondition.currentPage=1", "newwindow", "height=600, width=900, toolbar=no, menubar=yes, scrollbars=no, resizable=no, location=no, status=no");
　　 
	}
</script>
</head>
...
<div>
	...
	<table width="100%" border="0" cellspacing="0"  cellpadding="0" class="lists">
        ...
        <td class="lists_top"><font class="lists_fleft">商品流水列表</font></td>
		...
		<tr>
			...
			<input type="hidden" name="toReturn" value="yes"></input>
	  ...
	  <tr>
			...
			<td>创建时间 ：<input type="text" name="createTimeBegin" readonly="readonly" id="createTimeBegin" onFocus="WdatePicker({startDate:'%y-%M-01',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true,autoPickDate:true})"
										onchange="createDateOne()"	 value="${createTimeBegin}" />
			-<input type="text" name="createTimeEnd" readonly="readonly" id="createTimeEnd" onFocus="WdatePicker({startDate:'%y-%M-01',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true,autoPickDate:true})"
										onchange="createDateTwo()"	 value="${createTimeEnd}"/>
					...
					<s:if test="page.pageResult.size()!=0">
						...
						<s:iterator value="page.pageResult" id="cashrecord" status="status">
							<tr>
							           ...
							           <s:if test="#cashrecord.status==0">未支付</s:if>
							            <s:if test="#cashrecord.status==1">已支付</s:if>
							             <s:if test="#cashrecord.status==2">取消</s:if>
							              <s:if test="#cashrecord.status==3">已退费</s:if>
								...
								<td><a href="<%=contextPath%>/finance/backCashRecord!getSingleCommodity.action?queryCashRecordCondition.pcId=<s:property value="#cashrecord.id"/>">详情</a>
										<s:if test="#cashrecord.status==1">
											<s:if test="#cashrecord.cashRecordPrice>0">
											<a cashid="<s:property value="#cashrecord.id"/>" href="<%=contextPath%>/finance/refd!toInput.action?queryCashRecordCondition.pcId=<s:property value="#cashrecord.id"/>" class="cashid" />退费</a>
											<a href="#" onclick="openSellWayList(<s:property value="#cashrecord.cusId"/>,<s:property value="#cashrecord.id"/>);">更换此课程</a>
											</s:if>
										</s:if>
							</tr>
						</s:iterator>
