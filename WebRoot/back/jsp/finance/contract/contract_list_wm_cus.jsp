     ...
     if(startTime!=null&&endTime!=null)
     {
	     if(startTime>endTime)
	     {
	        alert("结束时间要大于开始时间!");
	   		...
	   		ObjectForm.action="<%=contextPath%>/finance/wmlcus.action";
			ObjectForm.submit();
	   	}
     }
   }
						...
						<form name="f1" method="post" action="bopayendTimeoks!getBooksList.action">
							...
							<table width="70%" border="0" cellspacing="1" cellpadding="0"
								...
								<tr>
									...
									<td class="lists_tleft" align="center">
										<img src="<%=contextPath%>/back/images/add_a.gif" /><a href="<%=contextPath%>/finance/regnum.action" target="_blank">注册订单量统计</a>
									</td>
								</tr>
							</table>
							</form>
								...
								<s:if test="page.pageResult.size()!=0">
									<s:iterator value="page.pageResult" id="contract" status="status">
										<tr>
											...
											<td>
												<s:property value="#contract.customer.cusWebFrom" />
											</td>
											<td>
												<s:property value="#contract.customer.cusWebAgent" />
											</td>
											...
											<td>
												...
												<script>
												var lin=<s:property value="#contract.contractCutSumMoney" />;
												document.write(lin);
												</script>											
												</s:if>
												<s:else>
											...
											<td>
												<s:if test="#contract.payType==2">
													...
													<s:elseif test="#contract.status==4">已退费</s:elseif>													
												</s:if>
												...
												<s:elseif test="#contract.payType==0">
													<s:if test="#contract.status==2">内部/代理商开通</s:if>
													<s:elseif test="#contract.status==4">内部/代理商开通</s:elseif>
												</s:elseif>
