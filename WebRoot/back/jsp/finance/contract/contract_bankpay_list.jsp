...
<html>
   ...
   function search(ObjectForm){
     ...
     if(startTime>endTime){
        ...
        return false;
   		...
   		ObjectForm.action="<%=contextPath%>/finance/backContract!getContractBankPayList.action";
		ObjectForm.submit();
   	}
     }
   }
   ...
   //银行汇款导出excel
  function excelBankPay(ObjectForm){
       var startTime=document.getElementById("startTime").value;
       var endTime=document.getElementById("endTime").value;
       if(startTime!=null&&endTime!=null){
	       if(startTime>endTime){
	          ...
	          return false;
	   		  ...
	   		  ObjectForm.action="<%=contextPath%>/finance/backContract!exportExcelFile.action";
			  ObjectForm.submit();
	   	  }
       }
   }
						...
						<td>
							...
							<table width="70%" border="0" cellspacing="1" cellpadding="0"
								...
								<tr>
									...
									<td><img src="<%=contextPath%>/back/images/down16_16.gif" /><a href="javascript:excelBankPay(document.f1)">导出银行汇款excel</a></td>
								</tr>
							</table>
							</div>
						</td>
									...
									<s:iterator value="page.pageResult" id="contract" status="status">
										<tr>
											...
											<s:elseif test="#contract.payType==7">
												<s:if test="#contract.status==0">未汇款</s:if>
												<s:elseif test="#contract.status==1">已汇款</s:elseif>
												<s:elseif test="#contract.status==4">已取消</s:elseif>
												<s:elseif test="#contract.status==3">已退费</s:elseif>
											</s:elseif>
											</td>
											...
											<s:elseif test="#contract.payType==7">
												<s:if test="#contract.status==0">
													<a href='<%=contextPath%>/finance/backContract!bankPayOpenCourse.action?contract.id=<s:property value="#contract.id"/>&contract.cusId=<s:property value="#contract.cusId"/>'
														onclick="return confirm('确认开通课程？')">等待开通课程</a>
