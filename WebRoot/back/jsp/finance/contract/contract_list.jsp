     ...
     if(startTime!=null&&endTime!=null)
     {
	     if(startTime>endTime)
	     {
	        alert("结束时间要大于开始时间!");
	   		...
	   		ObjectForm.action="<%=contextPath%>/finance/backContract!getContractList.action";
			ObjectForm.submit();
	   	}
     }
   }
	...
	<body onload="changeData()">
		<div>
			<form name="f1" method="post"
				action="bopayendTimeoks!getBooksList.action" id="form1">
							...
							<table width="70%" border="0" cellspacing="1" cellpadding="0"
								...
								<tr>
									<td>
										...
										<s:elseif test="fromURLType==6">
											...
											<s:hidden name="fromURLType" value="6"></s:hidden>
										</s:elseif>
										<s:else>
											<select name="contract.contractFromUrl" id="contractFromUrl"
												style="width: 155px">
									...
									<td>
										推 广 来 源：&nbsp;&nbsp;
										<%--
										<select name="contract.webFrom" id="webFrom"
											style="width: 155px">
											<option value="" id="op6">
												---请选择---
											</option>
											<s:iterator value="webFromStr" id="wf">
												<option value="<s:property value="#wf"/>">
													<s:property value="#wf" />
												</option>
											</s:iterator>
										</select>
										--%>
										<s:textfield name="contract.webFrom" id="webFrom" />
									</td>
								</tr>
								<tr>
									<td>
										销售 / 代理商：&nbsp;&nbsp;
										<%--
										<select name="contract.webAgent" id="webAgent"
											style="width: 155px">
											<option value="" id="op7">
												---请选择---
											</option>
											<s:iterator value="webAgentStr" id="wa">
												<option value="<s:property value="#wa"/>">
													<s:property value="#wa" />
												</option>
											</s:iterator>
										</select>
										 --%>
										 <s:textfield name="contract.webAgent" id="webAgent" />
									</td>
									<td>
										专 业：
								...
								<tr>
									...
									<td class="lists_tleft">
										...
										<img src="<%=contextPath%>/back/images/add_a.gif" />
										<a href="<%=contextPath%>/finance/invoice!getInvoiceInfo.action">新建发票</a>
									</td>
								</tr>
										...
										<tr>
											...
											<td id="666666">
												...
												<s:elseif test="#contract.payType==9">
													<s:if test="#contract.status==1">已付款</s:if>
													<s:if test="#contract.status==4">已取消</s:if>
												</s:elseif>
